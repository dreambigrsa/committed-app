...
import { Heart, MessageCircle, Share2, Plus, X, ExternalLink, MoreVertical, Edit2, Trash2, Image as ImageIcon, Flag, Camera, FileText, Video as VideoIcon } from 'lucide-react-native';
import { useApp } from '@/contexts/AppContext';
import { useTheme } from '@/contexts/ThemeContext';
import { Post, Advertisement, Comment as PostComment, ReelComment } from '@/types';
  ...
  const renderCardAd = (ad: Advertisement) => {
    ...
    const handleAdAddComment = async (postId: string, content: string, parentCommentId?: string, stickerId?: string, messageType?: 'text' | 'sticker') => {
      try {
        ...
        // Re-throw so CommentSheet can handle it
        throw error;
      }
    };
    ...
    return (
      <View key={`ad-card-${ad.id}`} style={styles.adCard}>
        ...
        {showAdComments === ad.id && (originalPost || originalReel) && (
          <CommentSheet
            contentId={originalPost?.id || originalReel?.id || ''}
            visible={true}
