...
export default function AdminEscalationRulesScreen() {
  ...
  const [triggerType, setTriggerType] = useState<'timeout' | 'user_request' | 'ai_detection' | 'manual'>('timeout');
  const [timeoutSeconds, setTimeoutSeconds] = useState(300);
  const [maxAttempts, setMaxAttempts] = useState(3);
  const [escalationStrategy, setEscalationStrategy] = useState<'sequential' | 'broadcast' | 'round_robin'>('sequential');
  const [fallbackRules, setFallbackRules] = useState<{ local_to_online?: boolean; role_expansion?: boolean }>({});
          ...
          {!showForm ? (
            ...
            <ScrollView style={styles.content}>
              <View style={styles.form}>
                ...
                <View style={styles.formGroup}>
                  <Text style={styles.label}>Priority (higher = evaluated first)</Text>
                  <TextInput
                    style={styles.input}
