...
export default function EditDateRequestScreen() {
  ...
  const dressCodes = dateOptions?.dressCodes || DEFAULT_DRESS_CODES.map(code => ({
    option_value: code,
    display_label: code.replace('_', ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()),
  }));
  ...
  useEffect(() => {
    if (dateRequest) {
      setDateTitle(dateRequest.date_title || '');
      setDateDescription(dateRequest.date_description || '');
      setDateLocation(dateRequest.date_location || '');
      setDateTime(dateRequest.date_time ? new Date(dateRequest.date_time) : new Date());
      setDateDurationHours(dateRequest.date_duration_hours?.toString() || '2');
      setDressCode(dateRequest.dress_code || '');
      setBudgetRange(dateRequest.budget_range || '');
      setExpenseHandling(dateRequest.expense_handling || 'split');
      setNumberOfPeople(dateRequest.number_of_people?.toString() || '2');
      setGenderPreference(dateRequest.gender_preference || 'everyone');
      setSpecialRequests(dateRequest.special_requests || '');
      setSuggestedActivities(dateRequest.suggested_activities || []);
    }
  }, [dateRequest]);
  ...
  const handleSubmit = () => {
    ...
    if (!dateRequestId) {
      Alert.alert('Error', 'Date request not found');
      return;
    }
        ...
        <View style={styles.section}>
          ...
          <View style={styles.optionsRow}>
            {dressCodes.map((option: any) => {
              ...
              const label = option.display_label || (typeof option === 'string' ? code.replace('_', ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()) : '');
              return (
                <TouchableOpacity
                  key={code}
                  style={[
