...
-- ============================================
-- This migration adds subscription/premium features
-- with admin-configurable pricing and limits
...
SELECT 
  ...
  feature,
  CASE feature
    WHEN 'daily_likes' THEN 10
    WHEN 'daily_super_likes' THEN 1
    WHEN 'rewinds' THEN 0
    WHEN 'boosts' THEN 0
    ELSE NULL
  END,
  'daily'
FROM subscription_plans sp
CROSS JOIN (VALUES ('daily_likes'), ('daily_super_likes'), ('rewinds'), ('boosts')) AS features(feature)
WHERE sp.name = 'free'
...
VALUES 
  ...
  ('free_plan_limits', '{"daily_likes": 10, "daily_super_likes": 1, "rewinds": 0, "boosts": 0}'::jsonb, 'Free plan feature limits')
