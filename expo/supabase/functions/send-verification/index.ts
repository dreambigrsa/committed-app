// @ts-nocheck
// POST /functions/v1/send-verification
// Body: { email? } or use session (Authorization header) to get email from auth.uid()
// Creates verify_email token (24h), sends email via Resend. Returns generic success.
...
const APP_WEB_DEV_URL = Deno.env.get("APP_WEB_DEV_URL") || "http://localhost:3000";
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!email || typeof email !== "string" || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return json(400, { success: true, message: "If this email is registered, you will receive a verification link." }, req);
    }
    ...
    if (recent && recent.length >= RATE_LIMIT_MAX_PER_EMAIL) {
      return json(200, { success: true, message: "If this email is registered, you will receive a verification link." }, req);
    }
    ...
    const baseUrl = APP_WEB_URL || APP_WEB_DEV_URL;
    const verifyUrl = `${baseUrl}/auth-callback?type=verify&token=${encodeURIComponent(rawToken)}`;
    const deepLink = `${APP_DEEPLINK_BASE}auth-callback?type=verify&token=${encodeURIComponent(rawToken)}`;
...
<div style="max-width:480px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.06)">
  <div style="background:linear-gradient(135deg,#1a73e8 0%,#1557b0 100%);padding:32px;text-align:center">
    ...
    <p style="margin:8px 0 0;font-size:14px;color:rgba(255,255,255,.9)">Verify your email</p>
  </div>
  <div style="padding:32px">
    <p style="margin:0 0 24px;color:#374151;line-height:1.6">Click the button below to verify your email address. This link expires in ${VERIFY_EXPIRY_HOURS} hours.</p>
    <p style="margin:0 0 16px"><a href="${verifyUrl}" style="display:inline-block;background:#1a73e8;color:#fff;text-decoration:none;padding:14px 28px;border-radius:12px;font-weight:600">Verify Email</a></p>
