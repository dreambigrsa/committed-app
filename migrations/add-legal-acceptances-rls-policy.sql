...
--
-- IMPORTANT: Run this in your Supabase SQL Editor to fix the RLS error

-- Check if table exists (should already exist, but be safe)
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Table user_legal_acceptances does not exist. Please create it first.';
  END IF;
END $$;
...
CREATE POLICY "Users can view own legal acceptances" ON public.user_legal_acceptances FOR SELECT
