-- Public registry search for visitors.
-- Only exposes verified relationships that opted into public visibility.
...
BEGIN
  ...
  WHERE r.status = 'verified'
    ...
    AND (
      ...
      OR regexp_replace(COALESCE(u.phone_number, ''), '\D', '', 'g') LIKE '%' || regexp_replace(normalized_query, '\D', '', 'g') || '%'
      OR lower(COALESCE(partner.full_name, r.partner_name, '')) LIKE '%' || normalized_query || '%'
      OR regexp_replace(COALESCE(partner.phone_number, r.partner_phone, ''), '\D', '', 'g') LIKE '%' || regexp_replace(normalized_query, '\D', '', 'g') || '%'
    )
