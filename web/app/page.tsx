...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f8faf7] text-slate-950">
      <Navbar />
      <main>
        <section className="relative min-h-[92svh] overflow-hidden bg-slate-950 text-white">
          ...
          <div className="relative mx-auto flex min-h-[92svh] max-w-7xl flex-col justify-end px-5 pb-10 pt-32 md:px-8 md:pb-14">
            <div className="max-w-3xl">
              <div className="inline-flex max-w-full items-start gap-2 rounded-md border border-white/22 bg-white/10 px-3 py-2 text-sm font-black leading-5 text-teal-100 backdrop-blur-md">
              ...
              <div className="mt-8 flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
                <Link href="/auth" className="inline-flex h-14 w-full items-center justify-center gap-2 rounded-md bg-teal-400 px-6 font-black text-slate-950 transition hover:bg-teal-300 sm:w-auto">
                  Open web app
                  <ArrowRight className="h-5 w-5" />
                </Link>
                <Link href="/sign-up" className="inline-flex h-14 w-full items-center justify-center rounded-md border border-white/28 bg-white/10 px-6 font-black text-white backdrop-blur-md transition hover:bg-white/18 sm:w-auto">
                  Create account
                </Link>
                <Link href={downloadHref} className="inline-flex h-14 w-full items-center justify-center rounded-md border border-white/28 px-6 font-black text-white transition hover:bg-white/10 sm:w-auto">
                  Download app
                </Link>
              </div>
            </div>
            ...
            <div className="mt-12 grid gap-3 md:grid-cols-3">
              {promises.map((promise) => (
                <div key={promise} className="min-w-0 rounded-md border border-white/16 bg-white/10 p-4 text-sm font-semibold leading-6 text-slate-100 backdrop-blur-md">
                  <CheckCircle2 className="mb-3 h-5 w-5 text-teal-300" />
                  <span className="break-words">{promise}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="experience" className="border-b border-slate-200 bg-[#f8faf7] py-16 md:py-20">
          <div className="mx-auto max-w-7xl px-5 md:px-8">
            ...
            <div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              {productMoments.map(({ icon: Icon, title, text }) => (
                <article key={title} className="group min-h-full rounded-lg border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-teal-200 hover:shadow-xl hover:shadow-teal-950/5">
                  <span className="grid h-12 w-12 place-items-center rounded-md bg-teal-50 text-teal-700 transition group-hover:bg-teal-500 group-hover:text-slate-950">
        ...
        <section id="singles" className="bg-white py-16 md:py-20">
          <div className="mx-auto max-w-7xl px-5 md:px-8">
            <div className="grid gap-5 lg:grid-cols-2">
              {journeys.map(({ label, title, text, href, cta, icon: Icon }) => (
                <article key={label} className="rounded-lg border border-slate-200 bg-[#fbfbf6] p-6 shadow-sm transition hover:-translate-y-1 hover:border-teal-200 hover:shadow-xl hover:shadow-slate-950/5 md:p-8">
                  ...
                  <Link href={href} className="mt-7 inline-flex h-12 w-full items-center justify-center gap-2 rounded-md bg-teal-500 px-5 font-black text-slate-950 transition hover:bg-teal-300 sm:w-auto">
                    {cta}
                    <ArrowRight className="h-5 w-5" />
                  </Link>
                </article>
        ...
        <section id="couples" className="bg-slate-950 py-16 text-white md:py-20">
          <div className="mx-auto grid max-w-7xl gap-10 px-5 md:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {trustLayers.map(([title, text]) => (
                <article key={title} className="rounded-lg border border-white/12 bg-white/10 p-5 shadow-lg shadow-slate-950/10 transition hover:border-teal-300/60 hover:bg-white/14">
                  <span className="grid h-10 w-10 place-items-center rounded-md bg-white/10 text-teal-300">
        ...
        <section id="support" className="bg-[#f8faf7] py-16 md:py-20">
          <div className="mx-auto max-w-7xl px-5 md:px-8">
            <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr] lg:items-stretch">
              <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm md:p-8">
                <UsersRound className="h-10 w-10 text-teal-700" />
                <h2 className="mt-6 text-3xl font-black tracking-normal md:text-4xl">One place for the whole relationship journey.</h2>
                ...
                <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                  <Link href="/auth" className="inline-flex h-12 items-center justify-center gap-2 rounded-md bg-slate-950 px-5 font-black text-white transition hover:bg-slate-800">
                    Enter Committed
                    <ArrowRight className="h-5 w-5" />
                  </Link>
                  <Link href="/download" className="inline-flex h-12 items-center justify-center rounded-md border border-slate-200 px-5 font-black text-slate-950 transition hover:border-teal-400 hover:bg-teal-50">
                    Download
                  </Link>
                </div>
              </div>
              <div className="grid gap-3">
                {[
                  ...
                  <div key={String(text)} className="flex min-h-[92px] items-center gap-4 rounded-lg border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-lg hover:shadow-slate-950/5">
          ...
          <div className="mx-auto max-w-7xl overflow-hidden rounded-lg bg-slate-950 text-white shadow-2xl shadow-slate-950/15">
            <div className="grid gap-8 p-6 md:p-8 lg:grid-cols-[1fr_auto] lg:items-center">
              <div>
                ...
                <h2 className="mt-5 max-w-3xl text-3xl font-black tracking-normal md:text-5xl">
              ...
              <div className="flex flex-col gap-3 sm:flex-row lg:flex-col">
                <Link href="/auth" className="inline-flex h-12 items-center justify-center gap-2 rounded-md bg-teal-400 px-5 font-black text-slate-950 transition hover:bg-teal-300">
                  Open web app
                  <ArrowRight className="h-5 w-5" />
                </Link>
                <Link href={downloadHref} className="inline-flex h-12 items-center justify-center rounded-md border border-white/20 px-5 font-black text-white transition hover:bg-white/10">
                  Download app
                </Link>
              </div>
            </div>
