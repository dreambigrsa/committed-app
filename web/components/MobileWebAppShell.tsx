...
export default function MobileWebAppShell({ initialTab = 'home' }: { initialTab?: string }) {
  ...
  const renderUserProfileRoute = () => {
    ...
    /** Own profile: viewer is in an active web session — show online even if `user_status` is stale/offline (web does not mirror native presence heartbeat yet). */
    const statusNorm = (routeProfileStatusType || '').toLowerCase();
    const online = isSelf || statusNorm === 'online';
